...
public:
    TraceEventDumper()
        : AutoTimer("Trace Event dumper")
    {
        SetTimeout(dumpTimeoutMS);
        Start();
    }

    virtual void Invoke() override { flushRecordings(); }

    static void flushRecordings()
    {
        const css::uno::Sequence<OUString> aEvents = comphelper::TraceEvent::getRecordingAndClear();
...
const ExtensionMap aWriterExtensionMap[] = { { "doc", "MS Word 97" },
...
const ExtensionMap aCalcExtensionMap[] = { { "csv", "Text - txt - csv (StarCalc)" },
...
const ExtensionMap aImpressExtensionMap[] = { { "fodp", "OpenDocument Presentation Flat XML" },
...
const ExtensionMap aDrawExtensionMap[] = { { "fodg", "draw_ODG_FlatXML" },
...
tools::Rectangle RectangleAndPart::SanitizedRectangle(tools::Long nLeft, tools::Long nTop,
...
void CallbackFlushHandler::CallbackData::updateRectangleAndPart(
...
getUndoManager(const css::uno::Reference<css::frame::XFrame>& rxFrame)
{
    ...
    if (xController.is())
    {
        ...
        if (xModel.is())
        {
            const css::uno::Reference<css::document::XUndoManagerSupplier> xSuppUndo(
                xModel, css::uno::UNO_QUERY_THROW);
            return css::uno::Reference<css::document::XUndoManager>(xSuppUndo->getUndoManager(),
                           ...
                           SvxLongLRSpaceItem* pPageLRMarginItem)
{
    ...
    SfxViewShell::Current()->GetDispatcher()->ExecuteList(
        SID_ATTR_PAGE_LRSPACE, SfxCallMode::RECORD, { pPageLRMarginItem });
}
                           ...
                           SvxLongULSpaceItem* pPageULMarginItem)
{
    ...
    SfxViewShell::Current()->GetDispatcher()->ExecuteList(
        SID_ATTR_PAGE_ULSPACE, SfxCallMode::RECORD, { pPageULMarginItem });
}
...
void ExecuteOrientationChange()
{
    ...
    std::unique_ptr<SvxSizeItem> pPageSizeItem(
    ...
    std::unique_ptr<SvxLongLRSpaceItem> pPageLRMarginItem(
        &pItem->Clone()->StaticWhichCast(SID_ATTR_PAGE_LRSPACE));

    SfxViewFrame::Current()->GetBindings().GetDispatcher()->QueryState(SID_ATTR_PAGE_ULSPACE,
                                                                       pItem);
    std::unique_ptr<SvxLongULSpaceItem> pPageULMarginItem(
        &pItem->Clone()->StaticWhichCast(SID_ATTR_PAGE_ULSPACE));

    {
        ...
        {
            SfxViewShell::Current()->GetDispatcher()->ExecuteList(
                SID_ATTR_PAGE_SIZE, SfxCallMode::RECORD, { pPageSizeItem.get(), pPageItem.get() });
        }
    }
    ...
    // if not, adjust margin values
    {
        ...
        if (nTmpPW > nPW)
        {
            if (nML <= nMR)
            {
                ExecuteMarginLRChange(pPageLRMarginItem->GetLeft(), nMR - (nTmpPW - nPW),
                                      pPageLRMarginItem.get());
            }
            else
            {
                ExecuteMarginLRChange(nML - (nTmpPW - nPW), pPageLRMarginItem->GetRight(),
                                      pPageLRMarginItem.get());
            }
        }
        ...
        if (nTmpPH > nPH)
        {
            if (nMT <= nMB)
            {
                ExecuteMarginULChange(pPageULMarginItem->GetUpper(), nMB - (nTmpPH - nPH),
                                      pPageULMarginItem.get());
            }
            else
            {
                ExecuteMarginULChange(nMT - (nTmpPH - nPH), pPageULMarginItem->GetLower(),
                                      pPageULMarginItem.get());
            }
        }
    }
...
extern "C"
{
    ...
    static void doc_moveSelectedParts(LibreOfficeKitDocument* pThis, int nPosition,
                                      bool bDuplicate);
    static char* doc_getPartName(LibreOfficeKitDocument* pThis, int nPart);
    static void doc_setPartMode(LibreOfficeKitDocument* pThis, int nPartMode);
    static void doc_paintTile(LibreOfficeKitDocument* pThis, unsigned char* pBuffer,
...
#ifdef IOS
    static void doc_paintTileToCGContext(LibreOfficeKitDocument* pThis, void* rCGContext,
                                         const int nCanvasWidth, const int nCanvasHeight,
                                         const int nTilePosX, const int nTilePosY,
                                         const int nTileWidth, const int nTileHeight);
#endif
    static void doc_paintPartTile(LibreOfficeKitDocument* pThis, unsigned char* pBuffer,
                                  const int nPart, const int nCanvasWidth, const int nCanvasHeight,
    ...
    static void doc_postKeyEvent(LibreOfficeKitDocument* pThis, int nType, int nCharCode,
                                 int nKeyCode);
    static void doc_setBlockedCommandList(LibreOfficeKitDocument* pThis, int nViewId,
                                          const char* blockedCommandList);

    static void doc_postWindowExtTextInputEvent(LibreOfficeKitDocument* pThis, unsigned nWindowId,
                                                int nType, const char* pText);
    static void doc_removeTextContext(LibreOfficeKitDocument* pThis, unsigned nLOKWindowId,
                                      int nCharBefore, int nCharAfter);
    static void doc_sendDialogEvent(LibreOfficeKitDocument* pThis,
                                    unsigned long long int nLOKWindowId, const char* pArguments);
    static void doc_postWindowKeyEvent(LibreOfficeKitDocument* pThis, unsigned nLOKWindowId,
    ...
    static void doc_postWindowMouseEvent(LibreOfficeKitDocument* pThis, unsigned nLOKWindowId,
                                         int nType, int nX, int nY, int nCount, int nButtons,
                                         int nModifier);
    static void doc_postWindowGestureEvent(LibreOfficeKitDocument* pThis, unsigned nLOKWindowId,
                                           const char* pType, int nX, int nY, int nOffset);
    ...
    static void doc_setWindowTextSelection(LibreOfficeKitDocument* pThis, unsigned nLOKWindowId,
                                           bool swap, int nX, int nY);
    ...
    static int doc_getSelectionTypeAndText(LibreOfficeKitDocument* pThis, const char* pMimeType,
                                           char** pText, char** pUsedMimeType);
    static int doc_getClipboard(LibreOfficeKitDocument* pThis, const char** pMimeTypes,
                                size_t* pOutCount, char*** pOutMimeTypes, size_t** pOutSizes,
                                char*** pOutStreams);
    static int doc_setClipboard(LibreOfficeKitDocument* pThis, const size_t nInCount,
                                const char** pInMimeTypes, const size_t* pInSizes,
                                const char** pInStreams);
    static bool doc_paste(LibreOfficeKitDocument* pThis, const char* pMimeType, const char* pData,
                          size_t nSize);
    ...
    static void doc_setClientZoom(LibreOfficeKitDocument* pThis, int nTilePixelWidth,
                                  int nTilePixelHeight, int nTileTwipWidth, int nTileTwipHeight);
    static void doc_setClientVisibleArea(LibreOfficeKitDocument* pThis, int nX, int nY, int nWidth,
                                         int nHeight);
    static void doc_setOutlineState(LibreOfficeKitDocument* pThis, bool bColumn, int nLevel,
                                    int nIndex, bool bHidden);
    ...
    static unsigned char* doc_renderFontOrientation(LibreOfficeKitDocument* pThis,
                                                    const char* pFontName, const char* pChar,
                                                    int* pFontWidth, int* pFontHeight,
                                                    int pOrientation);
    static unsigned char* doc_renderFont(LibreOfficeKitDocument* pThis, const char* pFontName,
    ...
    static void doc_paintWindowDPI(LibreOfficeKitDocument* pThis, unsigned nLOKWindowId,
                                   unsigned char* pBuffer, const int nX, const int nY,
                                   const int nWidth, const int nHeight, const double fDPIScale);
    ...
    static bool doc_renderSearchResult(LibreOfficeKitDocument* pThis, const char* pSearchResult,
                                       unsigned char** pBitmapBuffer, int* pWidth, int* pHeight,
                                       size_t* pByteSize);
...
rtl::Reference<LOKClipboard> forceSetClipboardForCurrentView(LibreOfficeKitDocument* pThis)
{
    ...
    pDoc->setClipboard(
        uno::Reference<datatransfer::clipboard::XClipboard>(xClip->getXI(), UNO_QUERY));
...
extern "C"
{
    ...
    void CallbackFlushHandler::TimeoutIdle::Invoke() { mHandler->Invoke(); }
    ...
    CallbackFlushHandler::~CallbackFlushHandler() { Stop(); }
    ...
    void CallbackFlushHandler::resetUpdatedType(int nType) { setUpdatedType(nType, false); }

    void CallbackFlushHandler::setUpdatedTypePerViewId(int nType, int nViewId, int nSourceViewId,
                                                       bool value)
    {
    ...
    void CallbackFlushHandler::libreOfficeKitViewCallbackWithViewId(int nType, const char* pPayload,
                                                                    int nViewId)
    {
    ...
    void
    CallbackFlushHandler::libreOfficeKitViewInvalidateTilesCallback(const tools::Rectangle* pRect,
                                                                    int nPart, int nMode)
    {
    ...
    void CallbackFlushHandler::libreOfficeKitViewUpdatedCallbackPerViewId(int nType, int nViewId,
                                                                          int nSourceViewId)
    {
        assert(isUpdatedTypePerViewId(nType));
        std::unique_lock<std::recursive_mutex> lock(m_mutex);
    ...
    void CallbackFlushHandler::queue(const int type, CallbackData& aCallbackData)
    {
        ...
        SAL_INFO("lok", "Queue: [" << type << "]: [" << aCallbackData.getPayload() << "] on "
        ...
        if (callbacksDisabled() && !bIsChartActive && !bIsComment)
        {
            ...
            if (type != LOK_CALLBACK_STATE_CHANGED && type != LOK_CALLBACK_INVALIDATE_TILES
                ...
                && type != LOK_CALLBACK_CURSOR_VISIBLE && type != LOK_CALLBACK_VIEW_CURSOR_VISIBLE
                && type != LOK_CALLBACK_TEXT_SELECTION && type != LOK_CALLBACK_TEXT_SELECTION_START
                && type != LOK_CALLBACK_TEXT_SELECTION_END && type != LOK_CALLBACK_MEDIA_SHAPE
            ...
            && aCallbackData.getPayload().find("\"hyperlink\": {}") == std::string::npos)
        {
            ...
            SAL_INFO("lok", "Skipping invalid event [" << type << "]: ["
                                                       << aCallbackData.getPayload() << "].");
            return;
        }
        ...
        switch (type)
        {
            ...
            case LOK_CALLBACK_CELL_AUTO_FILL_AREA:
            {
                ...
                {
                    SAL_INFO("lok", "Skipping queue duplicate ["
                                        << type << +"]: [" << aCallbackData.getPayload() << "].");
                    return;
                }
            }
            break;
        ...
        {
            const auto& posStart
                = std::find(m_queue1.rbegin(), m_queue1.rend(), LOK_CALLBACK_TEXT_SELECTION_START);
            auto posStart2 = toQueue2(posStart);
            if (posStart != m_queue1.rend())
                posStart2->clear();

            const auto& posEnd
                = std::find(m_queue1.rbegin(), m_queue1.rend(), LOK_CALLBACK_TEXT_SELECTION_END);
        ...
        else
        {
            switch (type)
            {
                ...
                case LOK_CALLBACK_FORM_FIELD_BUTTON:
                {
                    ...
                    const bool hyperLinkException
                        = type == LOK_CALLBACK_INVALIDATE_VISIBLE_CURSOR
                          && aCallbackData.getPayload().find("\"hyperlink\":\"\"")
                                 == std::string::npos
                          && aCallbackData.getPayload().find("\"hyperlink\": {}")
                                 == std::string::npos;
                ...
                case LOK_CALLBACK_STATE_CHANGED:
                {
                    ...
                    if (pos != std::string::npos)
                    {
                        ...
                        {
                            removeAll(type,
                                      [&name](const CallbackData& elemData) {
                                          return (
                                              elemData.getPayload().compare(0, name.size(), name)
                                              == 0);
                                      });
                        }
                    }
                ...
                case LOK_CALLBACK_GRAPHIC_SELECTION:
                {
                    ...
                    removeAll(
                        type, [](const CallbackData& elemData)
        ...
        SAL_INFO("lok", "Queued #" << (m_queue1.size() - 1) << " [" << type << "]: ["
                                   << aCallbackData.getPayload() << "] to have " << m_queue1.size()
                                   << " entries.");
...
#ifdef DBG_UTIL
        {
            ...
            assert(std::all_of(m_queue2.begin(), m_queue2.end(),
    ...
    bool CallbackFlushHandler::processInvalidateTilesEvent(int type, CallbackData& aCallbackData)
    {
        ...
        if (rcNew.isInfinite())
        {
            SAL_INFO("lok", "Have Empty [" << type << "]: [" << aCallbackData.getPayload()
                                           ...
                                           << ".");
            removeAll(LOK_CALLBACK_INVALIDATE_TILES,
                      [&rcNew](const CallbackData& elemData)
                      {
                          // Remove exiting if new is all-encompassing, or if of the same part.
                          return ((rcNew.m_nPart == -1
                                   || rcNew.m_nPart == elemData.getRectangleAndPart().m_nPart)
        ...
        else
        {
            ...
            SAL_INFO("lok", "Have [" << type << "]: [" << aCallbackData.getPayload()
            ...
            removeAll(LOK_CALLBACK_INVALIDATE_TILES,
                      [&rcNew](const CallbackData& elemData)
                      {
                          ...
                          {
                              SAL_INFO("lok", "Nothing to merge between new: " << rcNew.toString()
                                                                               << ", and old: "
                                                                               << rcOld.toString());
                              return false;
                          }
                          ...
                          else
                          {
                              ...
                              const bool bOverlap
                                  = !rcOverlap.IsEmpty() && rcOld.m_nMode == rcNew.m_nMode;
    ...
    bool CallbackFlushHandler::processWindowEvent(int type, CallbackData& aCallbackData)
    {
        ...
        if (aAction == "invalidate")
        {
            ...
            if (aRectStr.empty())
            {
                removeAll(LOK_CALLBACK_WINDOW,
                          [&nLOKWindowId](const CallbackData& elemData)
                          {
            ...
            else
            {
                ...
                removeAll(
                    LOK_CALLBACK_WINDOW,
                    [&aNewRect, &nLOKWindowId, &currentIsRedundant](const CallbackData& elemData)
                    {
                        const boost::property_tree::ptree& aOldTree = elemData.getJson();
                        if (aOldTree.get<std::string>("action", "") == "invalidate")
                        {
                            // Not possible that we encounter an empty rectangle here; we already handled this case above.
                            std::istringstream aOldRectStream(
                                aOldTree.get<std::string>("rectangle", ""));
                            tools::Long nOldLeft, nOldTop, nOldWidth, nOldHeight;
                            char nOldComma;
                            aOldRectStream >> nOldLeft >> nOldComma >> nOldTop >> nOldComma
                                >> nOldWidth >> nOldComma >> nOldHeight;
                            ...
                            if (nLOKWindowId == aOldTree.get<unsigned>("id", 0))
                            {
                                ...
                                {
                                    SAL_INFO("lok.dialog", "New rect [" << aNewRect.toString()
                                                                        << "] engulfs old ["
                                                                        << aOldRect.toString()
                                                                        << "]. Replacing old.");
                                    return true;
                                ...
                                {
                                    SAL_INFO("lok.dialog", "Old rect [" << aOldRect.toString()
                                                                        << "] engulfs new ["
                                                                        << aNewRect.toString()
                                                                        << "]. Skipping new.");
                                    // We have a rectangle in the queue already that makes the current Callback useless.
    ...
    void CallbackFlushHandler::enqueueUpdatedTypes()
    {
        if (m_updatedTypes.empty() && m_updatedTypesPerViewId.empty())
            return;
        assert(m_viewId >= 0);
        SfxViewShell* viewShell
            = SfxViewShell::GetFirst(false, [this](const SfxViewShell* shell)
        ...
        static const int orderedUpdatedTypesPerViewId[]
            = { LOK_CALLBACK_INVALIDATE_VISIBLE_CURSOR, LOK_CALLBACK_INVALIDATE_VIEW_CURSOR,
            ...
            for (int type : orderedUpdatedTypesPerViewId)
            {
                if (o3tl::make_unsigned(type) < types.size() && types[type].set)
                {
                    ...
                    if (sourceViewId != m_viewId)
                    {
                        ...
                        sourceViewShell = SfxViewShell::GetFirst(
                            false, [sourceViewId](const SfxViewShell* shell)
                            { return shell->GetViewShellId().get() == sourceViewId; });
                    }
                    if (sourceViewShell == nullptr)
                    {
                        SAL_INFO("lok", "View #" << sourceViewId
    ...
    void CallbackFlushHandler::enqueueUpdatedType(int type, const SfxViewShell* viewShell,
                                                  int viewId)
    {
        if (type == LOK_CALLBACK_INVALIDATE_VISIBLE_CURSOR)
        {
    ...
    void CallbackFlushHandler::Invoke()
    {
        ...
        for (; it1 != m_queue1.end(); ++it1, ++it2)
        {
            ...
            SAL_INFO("lok",
                     "processing event: [" << type << ',' << viewId << "]: [" << payload << "].");
            ...
            if (viewId == -1)
            {
                    ...
                    && (idx = payload.find('=')) != std::string::npos)
                {
                    ...
                    if (stateIt != m_lastStateChange.end())
                    {
                        ...
                        {
                            SAL_INFO("lok", "Skipping new state duplicate: [" << type << "]: ["
                                                                              << payload << "].");
                            continue;
                        }
                ...
                else
                {
                    ...
                    if (stateIt != m_states.end())
                    {
                        ...
                        {
                            SAL_INFO("lok",
                                     "Skipping duplicate [" << type << "]: [" << payload << "].");
                            continue;
                        }
            ...
            else // less common path for events relating to other views
            {
                ...
                if (statesIt != m_viewStates.end())
                {
                    ...
                    if (stateIt != states.end())
                    {
                        ...
                        if (stateIt->second == payload)
                        {
                            SAL_INFO("lok", "Skipping view duplicate [" << type << ',' << viewId
                                                                        << "]: [" << payload
                                                                        << "].");
                            continue;
                        }

                        SAL_INFO("lok", "Replacing an element in view states ["
                                            << type << ',' << viewId << "]: [" << payload << "].");
                        stateIt->second = payload;
                    }
                    else
                    {
                        SAL_INFO("lok", "Inserted a new element in view states: ["
                                            << type << ',' << viewId << "]: [" << payload << "]");
    ...
    void CallbackFlushHandler::removeViewStates(int viewId) { m_viewStates.erase(viewId); }
    ...
    static LibreOfficeKitDocument*
    lo_documentLoadWithOptions(LibreOfficeKit* pThis, const char* pURL, const char* pOptions);
    static void lo_registerCallback(LibreOfficeKit* pThis, LibreOfficeKitCallback pCallback,
                                    void* pData);
    static char* lo_getFilterTypes(LibreOfficeKit* pThis);
    static void lo_setOptionalFeatures(LibreOfficeKit* pThis, unsigned long long features);
    static void lo_setDocumentPassword(LibreOfficeKit* pThis, const char* pURL,
                                       const char* pPassword);
    static char* lo_getVersionInfo(LibreOfficeKit* pThis);
    static int lo_runMacro(LibreOfficeKit* pThis, const char* pURL);

    static bool lo_signDocument(LibreOfficeKit* pThis, const char* pUrl,
                                const unsigned char* pCertificateBinary,
                                const int nCertificateBinarySize,
                                const unsigned char* pPrivateKeyBinary,
                                const int nPrivateKeyBinarySize);

    static void lo_runLoop(LibreOfficeKit* pThis, LibreOfficeKitPollCallback pPollCallback,
                           LibreOfficeKitWakeCallback pWakeCallback, void* pData);

    static void lo_sendDialogEvent(LibreOfficeKit* pThis, unsigned long long int nLOKWindowId,
...
#ifdef IOS
    void paintTileToCGContext(ITiledRenderable* pDocument, void* rCGContext, const Size nCanvasSize,
                              const int nTilePosX, const int nTilePosY, const int nTileWidth,
                              const int nTileHeight)
    {
        ...
        pDocument->paintTile(*pDevice, nCanvasSize.Width(), nCanvasSize.Height(), nTilePosX,
                             nTilePosY, nTileWidth, nTileHeight);
    }

    void paintTileIOS(LibreOfficeKitDocument* pThis, unsigned char* pBuffer, const int nCanvasWidth,
                      const int nCanvasHeight, const double fDPIScale, const int nTilePosX,
                      const int nTilePosY, const int nTileWidth, const int nTileHeight)
    {
        CGContextRef pCGContext
            = CGBitmapContextCreate(pBuffer, nCanvasWidth, nCanvasHeight, 8, nCanvasWidth * 4,
        ...
        doc_paintTileToCGContext(pThis, (void*)pCGContext, nCanvasWidth, nCanvasHeight, nTilePosX,
                                 nTilePosY, nTileWidth, nTileHeight);
    ...
    void setFormatSpecificFilterData(std::u16string_view sFormat,
    ...
    lo_documentLoadWithOptions(LibreOfficeKit* pThis, const char* pURL, const char* pOptions)
    {
        ...
        try
        {
            ...
            auto const pair(
                pLib->mInteractionMap.insert(std::make_pair(aURL.toUtf8(), pInteraction)));
            ...
            if (!aMacroSecurityLevel.isEmpty())
            {
                ...
                SvNumberFormatter aFormatter(::comphelper::getProcessComponentContext(),
                                             LANGUAGE_ENGLISH_US);
...
#else
            const OUString aEnableMacrosExecution
                = extractParameter(aOptions, u"EnableMacrosExecution");
        ...
        */
            ...
            uno::Reference<lang::XComponent> xComponent
                = xComponentLoader->loadComponentFromURL(aURL, "_blank", 0, aFilterOptions);
            ...
            if (pLib->mpCallback)
            {
                ...
                pLib->mpCallback(LOK_CALLBACK_SIGNATURE_STATUS, OString::number(nState).getStr(),
                                 pLib->mpCallbackData);
            }
            ...
            aFontMappingUseData.erase(
                std::remove_if(
                    aFontMappingUseData.begin(), aFontMappingUseData.end(),
            ...
            aFontMappingUseData.erase(
                std::remove_if(
                    ...
                    [](OutputDevice::FontMappingUseItem x)
                    {
                        ...
                        if (x.mOriginalFont.indexOf('/') == -1)
                            for (const auto& j : x.mUsedFonts)
                                if ((x.mOriginalFont == "Arial" && j.startsWith("Liberation Sans/"))
                                    ...
                                    || (x.mOriginalFont == "Cambria" && j.startsWith("Caladea/"))
                                    || (x.mOriginalFont == "Calibri" && j.startsWith("Carlito/"))
                                    ...
                                    (x.mOriginalFont == "Symbol" && j.startsWith("OpenSymbol/")))
                                {
                                    return true;
                                }
    ...
    static int lo_runMacro(LibreOfficeKit* pThis, const char* pURL)
    {
        ...
        if (xFactory.is())
        {
            ...
            xDP.set(xSFactory->createInstance("com.sun.star.comp.sfx2.SfxMacroLoader"),
                    uno::UNO_QUERY);
            ...
            if (aErr.Name == "ErrorCode")
            {
                ...
                pLib->maLastExceptionMsg = "An error occurred running macro (error code: "
                                           + OUString::number(nErrCode) + ")";
                                ...
                                const int nPrivateKeyBinarySize)
    {
        ...
        std::string aCertificateString(reinterpret_cast<const char*>(pCertificateBinary),
        ...
        std::string aPrivateKeyString(reinterpret_cast<const char*>(pPrivateKeyBinary),
        ...
        else
        {
            ...
            std::copy(pPrivateKeyBinary, pPrivateKeyBinary + nPrivateKeyBinarySize,
        ...
        uno::Reference<xml::crypto::XCertificateCreator> xCertificateCreator(xSecurityEnvironment,
                                                                             uno::UNO_QUERY);
        ...
        try
        {
            ...
            if ((aIndex = aFilterOptions.indexOf(",Watermark=")) >= 0)
            {
                ...
                aFilterOptions = OUString::Concat(aFilterOptions.subView(0, aIndex))
                                 + aFilterOptions.subView(bIndex + 12);
            }
            ...
            if ((aIndex = aFilterOptions.indexOf(",FullSheetPreview=")) >= 0)
            {
                ...
                aFilterOptions = OUString::Concat(aFilterOptions.subView(0, aIndex))
                                 + aFilterOptions.subView(bIndex + 16);
            }
            ...
            if ((aIndex = aFilterOptions.indexOf(",PDFVer=")) >= 0)
            {
                ...
                aFilterOptions = OUString::Concat(aFilterOptions.subView(0, aIndex))
                                 + aFilterOptions.subView(bIndex + 9);
 ...
 */
    static void doc_iniUnoCommands()
    {
        ...
        OUString sUnoCommands[] = { OUString(".uno:AlignLeft"),
    ...
    static void doc_setPartImpl(LibreOfficeKitDocument* pThis, int nPart,
                              ...
                              const int nTilePosY, const int nTileWidth, const int nTileHeight)
    {
        ...
        SAL_INFO("lok.tiledrendering", "paintTile: painting ["
                                           << nTileWidth << "x" << nTileHeight << "]@(" << nTilePosX
        ...
        paintTileIOS(pThis, pBuffer, nCanvasWidth, nCanvasHeight, fDPIScaleX, nTilePosX, nTilePosY,
        ...
        pDoc->paintTile(*pDevice, nCanvasWidth, nCanvasHeight, nTilePosX, nTilePosY, nTileWidth,
                        nTileHeight);
                                         ...
                                         const int nTileWidth, const int nTileHeight)
    {
        ...
        SAL_INFO("lok.tiledrendering", "paintTileToCGContext: painting ["
                                           << nTileWidth << "x" << nTileHeight << "]@(" << nTilePosX
                                           << ", " << nTilePosY << ") to [" << nCanvasWidth << "x"
                                           << nCanvasHeight << "]px");
        ...
        paintTileToCGContext(pDoc, rCGContext, aCanvasSize, nTilePosX, nTilePosY, nTileWidth,
                             nTileHeight);
    }
                                  ...
                                  const int nTileHeight)
    {
        ...
        SAL_INFO("lok.tiledrendering", "paintPartTile: painting @ "
                                           << nPart << " [" << nTileWidth << "x" << nTileHeight
        ...
        try
        {
            ...
            doc_paintTile(pThis, pBuffer, nCanvasWidth, nCanvasHeight, nTilePosX, nTilePosY,
                          nTileWidth, nTileHeight);
                                     ...
                                     LibreOfficeKitCallback pCallback, void* pData)
    {
        ...
        pDocument->mpCallbackFlushHandlers[nView]
            = std::make_shared<CallbackFlushHandler>(pThis, pCallback, pData);
        ...
        if (pCallback != nullptr)
        {
            ...
            {
                pDocument->mpCallbackFlushHandlers[nView]->setViewId(
                                                ...
                                                int nType, const char* pText)
    {
        ...
        SfxLokHelper::postExtTextEventAsync(
            pWindow, nType, OUString::fromUtf8(std::string_view(pText, strlen(pText))));
    }
                                      ...
                                      int nCharBefore, int nCharAfter)
    {
        ...
        if (nCharBefore > 0)
        {
            ...
            if (nLOKWindowId == 0)
            {
                ...
                SfxLokHelper::postKeyEventAsync(pWindow, LOK_KEYEVENT_KEYINPUT, 8, 1283,
                                                nCharBefore - 1);
        }
        ...
        if (nCharAfter > 0)
        {
            ...
            if (nLOKWindowId == 0)
            {
                ...
                SfxLokHelper::postKeyEventAsync(pWindow, LOK_KEYEVENT_KEYINPUT, 46, 1286,
                                                nCharAfter - 1);
        }
    }
...
*/
    ...
    public:
        ...
        dispatchFinished(const css::frame::DispatchResultEvent& rEvent) override
        {
    ...
    static void lcl_sendDialogEvent(unsigned long long int nWindowId, const char* pArguments)
    {
        ...
        try
        {
            ...
            if (!bFoundWeldedControl)
                bFoundWeldedControl = jsdialog::ExecuteAction(
                    std::to_string(nCurrentShellId) + "sidebar", sControlId, aMap);
            if (!bFoundWeldedControl)
                bFoundWeldedControl = jsdialog::ExecuteAction(
                    std::to_string(nCurrentShellId) + "notebookbar", sControlId, aMap);
            if (!bFoundWeldedControl)
                bFoundWeldedControl = jsdialog::ExecuteAction(
                    std::to_string(nCurrentShellId) + "formulabar", sControlId, aMap);
    ...
    static void doc_sendDialogEvent(LibreOfficeKitDocument* /*pThis*/,
                                    unsigned long long int nWindowId, const char* pArguments)
    {
        lcl_sendDialogEvent(nWindowId, pArguments);
    }

    static void lo_sendDialogEvent(LibreOfficeKit* /*pThis*/, unsigned long long int nWindowId,
                                   const char* pArguments)
    {
        lcl_sendDialogEvent(nWindowId, pArguments);
    }
                                   ...
                                   const char* pArguments, bool bNotifyWhenFinished)
    {
        ...
        std::vector<beans::PropertyValue> aPropertyValuesVector(
        ...
        if (gImpl && aCommand == ".uno:Save")
        {
            ...
            aPropertyValuesVector.erase(
                std::remove_if(aPropertyValuesVector.begin(), aPropertyValuesVector.end(),
        ...
        else if (gImpl && aCommand == ".uno:TransformDialog")
        {
            ...
            else if (const SdrView* pView = pViewShell->GetDrawView())
            {
                ...
                {
                    bNeedConversion
                        = (pOutputDevice->GetMapMode().GetMapUnit() == MapUnit::Map100thMM);
                }
            }
            ...
            if (bNeedConversion)
            {
                ...
                {
                    if (rPropValue.Name == "TransformPosX" || rPropValue.Name == "TransformPosY"
                        ...
                        || rPropValue.Name == "TransformRotationY")
                    {
                        ...
                        value = OutputDevice::LogicToLogic(value, MapUnit::MapTwip,
                                                           MapUnit::Map100thMM);
                        rPropValue.Value <<= value;
                    }
                }
            ...
            if (aChartHelper.GetWindow() && aPropertyValuesVector.size() > 0)
            {
                if (aPropertyValuesVector[0].Name != "Action")
                {
                    ...
                    int nLeft
                        = o3tl::convert(aChartBB.Left(), o3tl::Length::twip, o3tl::Length::mm100);
                    int nTop
                        = o3tl::convert(aChartBB.Top(), o3tl::Length::twip, o3tl::Length::mm100);
                ...
                css::uno::Reference<css::frame::XDispatch>& aChartDispatcher
                    = aChartHelper.GetXDispatcher();
        ...
        {
            bResult = comphelper::dispatchCommand(
                aCommand, comphelper::containerToSequence(aPropertyValuesVector),
    ...
    static bool
    getFromTransferrable(const css::uno::Reference<css::datatransfer::XTransferable>& xTransferable,
                         const OString& aInMimeType, OString& aRet);

    static bool
    encodeImageAsHTML(const css::uno::Reference<css::datatransfer::XTransferable>& xTransferable,
                      const OString& aMimeType, OString& aRet)
    {
        ...
        aRet = "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.0 Transitional//EN\">\n"
               ...
               + getGenerator().toUtf8()
               + "\"/>"
                 "</head><body><img src=\"data:"
               + aMimeType + ";base64," + aBase64Data + "\"/></body></html>";

        return true;
    }

    static bool
    encodeTextAsHTML(const css::uno::Reference<css::datatransfer::XTransferable>& xTransferable,
                     const OString& aMimeType, OString& aRet)
    {
        if (!getFromTransferrable(xTransferable, aMimeType, aRet))
            return false;
        ...
        aRet = "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.0 Transitional//EN\">\n"
               ...
               "<meta http-equiv=\"content-type\" content=\"text/html; charset=utf-8\"/><meta "
               "name=\"generator\" content=\""
               + getGenerator().toUtf8() + "\"/></head><body><pre>" + aRet + "</pre></body></html>";
                         ...
                         const OString& aInMimeType, OString& aRet)
    {
        ...
        {
            SetLastExceptionMsg("Unsupported flavor " + aFlavor.MimeType + " exception "
                                + e.Message);
            return false;
        }
        catch (const css::uno::Exception& e)
        {
            SetLastExceptionMsg("Exception getting " + aFlavor.MimeType + " exception "
                                + e.Message);
            return false;
        }
        ...
        else
        {
            ...
            aAny >>= aSequence;
            aRet = OString(reinterpret_cast<const char*>(aSequence.getConstArray()),
                           aSequence.getLength());
        }
    ...
    static int doc_getSelectionType(LibreOfficeKitDocument* pThis)
    {
        ...
        css::uno::Reference<css::datatransfer::XTransferable2> xTransferable2(xTransferable,
                                                                              css::uno::UNO_QUERY);
                                ...
                                char*** pOutStreams)
    {
        ...
        *pOutStreams = nullptr;
        ...
        {
            const uno::Sequence<css::datatransfer::DataFlavor> flavors
    ...
    {
        OUString aFoundFont(::rtl::Uri::decode(OStringToOUString(aFontName, RTL_TEXTENCODING_UTF8),
    ...
    static char* getStyles(LibreOfficeKitDocument* pThis, const char* pCommand)
    {
        ...
        boost::property_tree::ptree aTree;
        aTree.put("commandName", pCommand);
        uno::Reference<css::style::XStyleFamiliesSupplier> xStyleFamiliesSupplier(
            pDocument->mxComponent, uno::UNO_QUERY);
        ...
        static const std::vector<OUString> aWriterStyles
            = { "Text body", "Quotations", "Title",    "Subtitle",
        ...
        for (OUString const& sStyleFam : aStyleFamilies)
        {
            ...
            uno::Reference<container::XNameAccess> xStyleFamily(
                xStyleFamilies->getByName(sStyleFam), uno::UNO_QUERY);
            ...
            if (sStyleFam == "ParagraphStyles" && doc_getDocumentType(pThis) == LOK_DOCTYPE_TEXT)
            {
        ...
        // Header & Footer Styles
        {
                ...
                && (xStyleFamilies->getByName(sPageStyles) >>= xContainer))
            {
                ...
                for (OUString const& sName : aSeqNames)
                {
                    ...
                    if (xProperty.is()
                        && (xProperty->getPropertyValue("IsPhysical") >>= bIsPhysical)
                        && bIsPhysical)
                    {
    ...
    static char* getTrackedChanges(LibreOfficeKitDocument* pThis)
    {
        ...
        uno::Reference<document::XRedlinesSupplier> xRedlinesSupplier(pDocument->mxComponent,
                                                                      uno::UNO_QUERY);
        tools::JsonWriter aJson;
        ...
        if (doc_getDocumentType(pThis) != LOK_DOCTYPE_TEXT && xRedlinesSupplier.is())
        {
            ...
            {
                uno::Reference<beans::XPropertySet> xRedline(xRedlines->nextElement(),
                                                             uno::UNO_QUERY);
    ...
    static char* doc_getCommandValues(LibreOfficeKitDocument* pThis, const char* pCommand)
    {
        ...
        static const std::initializer_list<std::u16string_view> vForward
            = { u"TextFormFields", u"SetDocumentProperties", u"Bookmarks", u"GetOutline" };
        ...
        else if (aCommand.startsWith(aSheetGeometryData))
        {
            ...
            if (aCommand.getLength() > aSheetGeometryData.getLength())
            {
                ...
                do
                {
                    ...
                    bool bEnableFlag = aValue.isEmpty() || aValue.equalsIgnoreAsciiCase("true")
                                       || aValue.toInt32() > 0;
                    if (!bEnableFlag)
                        continue;
    ...
    static bool doc_getViewIds(SAL_UNUSED_PARAMETER LibreOfficeKitDocument* pThis, int* pArray,
                               size_t nSize)
    {
    ...
    static void doc_setViewLanguage(SAL_UNUSED_PARAMETER LibreOfficeKitDocument* /*pThis*/, int nId,
                                    const char* language)
    {
    ...
    unsigned char* doc_renderFontOrientation(SAL_UNUSED_PARAMETER LibreOfficeKitDocument* /*pThis*/,
                                             const char* pFontName, const char* pChar,
                                             int* pFontWidth, int* pFontHeight, int pOrientation)
    {
        ...
        auto aFont
            = FindFont_FallbackToDefault(OStringToOUString(pFontName, RTL_TEXTENCODING_UTF8));
        ...
        aDevice->SetOutputSizePixelScaleOffsetAndLOKBuffer(Size(nFontWidth, nFontHeight),
                                                           Fraction(1.0), Point(), pBuffer);

        if (*pFontWidth > 0 && *pFontHeight > 0)
        {
            DrawTextFlags const nStyle
                = DrawTextFlags::Center | DrawTextFlags::VCenter | DrawTextFlags::MultiLine
    ...
    static void doc_paintWindowDPI(LibreOfficeKitDocument* pThis, unsigned nLOKWindowId,
                                   ...
                                   const int nWidth, const int nHeight, const double fDPIScale)
    {
        doc_paintWindowForView(pThis, nLOKWindowId, pBuffer, nX, nY, nWidth, nHeight, fDPIScale,
                               -1);
    }
                                       ...
                                       int viewId)
    {
        ...
        CGContextRef cgc = CGBitmapContextCreate(
            pBuffer, nWidth, nHeight, 8, nWidth * 4, CGColorSpaceCreateDeviceRGB(),
        ...
        pDevice->SetOutputSizePixelScaleOffsetAndLOKBuffer(Size(nWidth, nHeight), Fraction(1.0),
                                                           Point(), pBuffer);
    ...
    static void doc_postWindow(LibreOfficeKitDocument* /*pThis*/, unsigned nLOKWindowId,
                               int nAction, const char* pData)
    {
        ...
        else if (nAction == LOK_WINDOW_PASTE)
        {
            ...
            {
                aArgs.size() == 2 && aArgs[0].Name == "MimeType" && (aArgs[0].Value >>= aMimeType)
            ...
            {
                uno::Reference<datatransfer::XTransferable> xTransferable(
                    new LOKTransferable(aMimeType, aData));
                                   ...
                                   const int nCertificateBinarySize)
    {
        ...
        SAL_INFO("lok", "Certificate Added = IssuerName: " << xCertificate->getIssuerName()
                                                           << " SubjectName: "
                                       ...
                                       size_t* pByteSize)
    {
        ...
        int aPixelWidth
            = o3tl::convert(aRangeUnion.getWidth(), o3tl::Length::twip, o3tl::Length::px);
        int aPixelHeight
            = o3tl::convert(aRangeUnion.getHeight(), o3tl::Length::twip, o3tl::Length::px);
        ...
        *pByteSize = nByteSize;
        ...
        doc_paintTile(pThis, pBuffer, aPixelWidth, aPixelHeight, aRangeUnion.getMinX(),
                      aRangeUnion.getMinY(), aRangeUnion.getWidth(), aRangeUnion.getHeight());
    ...
    static void lo_freeError(char* pFree) { free(pFree); }
    ...
    static char* lo_getFilterTypes(LibreOfficeKit* pThis)
    {
        ...
        uno::Reference<container::XNameAccess> xTypeDetection(
            xSFactory->createInstance("com.sun.star.document.TypeDetection"), uno::UNO_QUERY);
    ...
    {
        OString aBuffer
            = "Unexpected dialog: " + OUStringToOString(rAction, RTL_TEXTENCODING_ASCII_US)
              + " Error: " + OUStringToOString(rError, RTL_TEXTENCODING_ASCII_US);
    ...
    static void
    lo_status_indicator_callback(void* data,
                                 comphelper::LibreOfficeKit::statusIndicatorCallbackType type,
                                 int percent, const char* pText)
    {
        ...
        switch (type)
        {
            ...
            case comphelper::LibreOfficeKit::statusIndicatorCallbackType::SetValue:
                pLib->mpCallback(LOK_CALLBACK_STATUS_INDICATOR_SET_VALUE,
                                 OUString(OUString::number(percent)).toUtf8().getStr(),
                                 pLib->mpCallbackData);
                break;
            case comphelper::LibreOfficeKit::statusIndicatorCallbackType::Finish:
                pLib->mpCallback(LOK_CALLBACK_STATUS_INDICATOR_FINISH, nullptr,
                                 pLib->mpCallbackData);
                break;
        }
    }
    ...
    static void preloadData()
    {
        ...
        css::uno::Reference<css::linguistic2::XLinguServiceManager> xLngSvcMgr
            = css::linguistic2::LinguServiceManager::create(
                comphelper::getProcessComponentContext());
        css::uno::Reference<linguistic2::XSpellChecker> xSpellChecker(
            xLngSvcMgr->getSpellChecker());

        std::cerr << "Preloading dictionaries: ";
        css::uno::Reference<linguistic2::XSupportedLocales> xSpellLocales(
            xSpellChecker, css::uno::UNO_QUERY_THROW);
        ...
        css::uno::Reference<linguistic2::XSupportedLocales> xThesLocales(xSpellChecker,
        ...
        for (const auto& aLocale : std::as_const(aLocales))
        {
            ...
            nLang = MsLangId::resolveSystemLanguageByScriptType(
                LanguageTag::convertToLanguageType(aLocale, false), LATIN);
            OutputDevice::GetDefaultFont(DefaultFontType::LATIN_SPREADSHEET, nLang,
                                         GetDefaultFontFlags::OnlyOne);
            nLang = MsLangId::resolveSystemLanguageByScriptType(
                LanguageTag::convertToLanguageType(aLocale, false), ASIAN);
            OutputDevice::GetDefaultFont(DefaultFontType::CJK_SPREADSHEET, nLang,
                                         GetDefaultFontFlags::OnlyOne);
            nLang = MsLangId::resolveSystemLanguageByScriptType(
                LanguageTag::convertToLanguageType(aLocale, false), COMPLEX);
...
#pragma GCC diagnostic pop
    ...
    void setCertificateDir()
    {
        ...
        if (pEnvVarString)
        {
            OUString aCertificateDatabasePath
                = OStringToOUString(pEnvVarString, RTL_TEXTENCODING_UTF8);
            try
            {
                std::shared_ptr<comphelper::ConfigurationChanges> pBatch(
                    comphelper::ConfigurationChanges::create());
                officecfg::Office::Common::Security::Scripting::CertDir::set(
                    aCertificateDatabasePath, pBatch);
                officecfg::Office::Common::Security::Scripting::ManualCertDir::set(
                    aCertificateDatabasePath, pBatch);
                pBatch->commit();
            }
            catch (uno::Exception const& rException)
            {
                SAL_WARN("lok", "Failed to set the NSS certificate database directory: "
                                    << rException.Message);
            }
        }
    }
    ...
    void setLanguageToolConfig()
    {
        ...
        if (pEnabled && pBaseUrlString)
        {
            ...
            OUString aRestProtocol
                = pRestProtocol ? OStringToOUString(pRestProtocol, RTL_TEXTENCODING_UTF8) : "";
            try
            {
                             ...
                             const char* pUserProfileUrl)
    {
        ...
        { // cf. string lifetime for preinit
            ...
            if (pOptions)
                aOpts = comphelper::string::split(
                    OUString(pOptions, strlen(pOptions), RTL_TEXTENCODING_UTF8), ':');
        ...
        {
            OUString url(pUserProfileUrl, strlen(pUserProfileUrl), RTL_TEXTENCODING_UTF8);
            OUString path;
            if (url.startsWithIgnoreAsciiCase("vnd.sun.star.pathname:", &path))
            {
                OUString url2;
                osl::FileBase::RC e = osl::FileBase::getFileURLFromSystemPath(path, url2);
        ...
        int fd
            = open([[bundlePath stringByAppendingPathComponent:@"ICU.dat"] UTF8String], O_RDONLY);
        if (fd == -1)
            NSLog(@"Could not open ICU data file %s",
        ...
        else
        {
            ...
            {
                void* icudata
                    = mmap(0, (size_t)st.st_size, PROT_READ, MAP_FILE | MAP_PRIVATE, fd, 0);
        ...
        try
        {
            if (eStage != SECOND_INIT)
            {
                ...
                if (eStage == PRE_INIT)
                {
                    ...
                    xContext->getValueByName("/singletons/com.sun.star.lang.theServiceManager")
                        >>= xService;
                    if (!xService.is())
                        throw css::uno::DeploymentException(
...
// Turn off quick editing on IOS and ANDROID
    ...
    temporaryHackToInvokeCallbackHandlers(LibreOfficeKitDocument* pThis)
    {
        SolarMutexGuard aGuard;
        LibLODocument_Impl* pDocument = static_cast<LibLODocument_Impl*>(pThis);
