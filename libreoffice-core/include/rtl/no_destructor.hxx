...
// has a non-trivial destructor. Storing a `T` in a `rtl::NoDestructor<T>` will
...
// `rtl::NoDestructor<T>` prevents the latter.
//
// ## Caveats
//
// - Must only be used as a function-local static variable. Declaring a global
//   variable of type `rtl::NoDestructor<T>` will still generate a global
...
//   caching it for the lifetime of the program. Though `rtl::NoDestructor<T>`
//   does not heap allocate, the compiler still reserves space in bss for
//   storing `T`, which costs memory at runtime.
//
// - If `T` is trivially destructible, do not use `rtl::NoDestructor<T>`:
//
//     const uint64_t GetUnstableSessionSeed() {
//       // No need to use `rtl::NoDestructor<T>` as `uint64_t` is trivially
...
//   static const rtl::NoDestructor<std::string> s("Hello world!");
//   return *s;
// }
//
...
//   static const rtl::NoDestructor<std::string> nonce([] {
//     std::string s(16);
